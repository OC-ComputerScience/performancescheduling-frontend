...
import {generateTimeSlots, subtractTimes} from "../../../../composables/timeManipulator.js";
...
function sortData() {
  ...
  var timeSlots = generateTimeSlots(props.eventData.startTime, props.eventData.endTime, 30);
  timeSlots.pop();
  ...
  props.availData.forEach(element => {
    ...
    var numberOfAvailabilities = subtractTimes(element.startTime, element.endTime) / 30;
    ...
    if (element.userRole.role.role == "Faculty"){
      for (var i = 0; i < numberOfAvailabilities; i++){
        tableData.value[beginIndex + i].faculty.push({firstName: element.userRole.user.firstName, lastName: element.userRole.user.lastName });
      }
    }
    // If the element's role is an accompanist, add their first and last name to the accompanist array
      ...
      for (var i = 0; i < numberOfAvailabilities; i++){
        tableData.value[beginIndex + i].accompanist.push({firstName: element.userRole.user.firstName, lastName: element.userRole.user.lastName });
      }
    }
  });
    ...
    for (var i = 0; i <  timeSlots.length - 1 && !placed; i++){
      if (element.startTime < timeSlots[i].time){
        element.studentInstrumentSignups.forEach((signup) => {
          tableData.value[i - 1].student.push({firstName: signup.studentInstrument.studentRole.user.firstName, lastName: signup.studentInstrument.studentRole.user.lastName})
        });
        placed = true;
      }
    }
